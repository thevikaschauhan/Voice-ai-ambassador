...
def test_allowed_sentence_becomes_speakable(allowed, patterns, forms, vocatives):
    result = process_sentence(
        "Skyrise starts at AED 985,000.", "en", allowed, patterns, forms, vocatives
    )
...
def test_invented_figure_never_becomes_speakable(allowed, patterns, forms, vocatives):
    result = process_sentence(
        "It starts at AED 800,000.", "en", allowed, patterns, forms, vocatives
    )
...
def test_prohibited_language_never_becomes_speakable(
    allowed, patterns, forms, vocatives
):
    result = process_sentence(
        "This is a risk-free investment.", "en", allowed, patterns, forms, vocatives
    )
    assert isinstance(result, GuardrailViolation)
    assert result.validator == "prohibited_language"


def test_guarantee_holds_in_arabic_digits(allowed, patterns, forms, vocatives):
    # The numeric guardrail is language-agnostic: Arabic-Indic digits are
    # extracted and checked identically
    result = process_sentence(
        "يبدأ السعر من ٨٠٠٬٠٠٠ درهم", "ar", allowed, patterns, forms, vocatives
    )
    assert isinstance(result, GuardrailViolation)
