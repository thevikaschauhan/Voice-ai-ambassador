...
def test_publication_requires_the_exact_reviewed_set_and_preserves_closures():
    ...
    chunks = [KnowledgeReviewChunk(id=i, ordinal=n, body="Reviewed prose.",
                                  retrieval_scope="inventory_governed" if n == 2 else "admin_only")
              for n, i in enumerate(ids)]
    selection = [KnowledgeScopeSelection(chunk_id=i, action="general_knowledge") for i in ids]
    ...
    selection[0] = KnowledgeScopeSelection(chunk_id=ids[0], action="project_knowledge", project_id="missing")
...
async def seed(repo):
    ...
    result = await store_document(repo, title="Review fixture", parsed=parse_document(None, None, pasted=text))
    return result["id"]
...
async def request_for(repo, document_id):
    ...
    return KnowledgePublicationRequest(
        request_id=uuid4(), expected_revision=document["revision"],
        expected_review_token=document["review_token"], confirmed=True,
        selections=[{"chunk_id": str(c["id"]), "action": "general_knowledge"} for c in document["chunks"]],
    )
...
async def test_atomic_publish_keeps_numbers_withheld_and_retries_once(repository):
    ...
    rows = await repo.search_knowledge("terraces")
    assert len(rows) == 1
    async with repo._pool.acquire() as c:
        assert await c.fetchval("SELECT count(*) FROM knowledge_document_publications") == 1
        assert await c.fetchval("SELECT count(*) FROM knowledge_chunk_reviews") == 2
    changed = KnowledgePublicationRequest.model_validate({**request.model_dump(), "confirmed": False})
    with pytest.raises(ValueError):
        await repo.publish_document(doc_id, changed)


async def test_stale_figure_review_cannot_publish_and_exclusions_stay_closed(repository):
    ...
    fresh.selections[1] = KnowledgeScopeSelection(chunk_id=fresh.selections[1].chunk_id, action="admin_only")
    await repo.publish_document(doc_id, fresh)
    assert await repo.search_knowledge("minutes") == []
...
async def test_failed_publication_rolls_back_all_reviews(repository):
    ...
    assert all(c["prompt_body"] is None for c in await repo.get_chunks(doc_id, revision=1))


async def test_publication_route_is_bearer_protected_and_publishes(repository, monkeypatch):
    ...
    async with AsyncClient(transport=ASGITransport(app=admin_api.app), base_url="http://admin") as client:
        url = f"/v1/knowledge/documents/{doc_id}/publish"
        assert (await client.post(url, json=request.model_dump(mode="json"))).status_code == 401
        response = await client.post(url, json=request.model_dump(mode="json"), headers={"Authorization": f"Bearer {token}"})
