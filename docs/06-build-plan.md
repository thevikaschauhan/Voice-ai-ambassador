...
| P2-S05 | One-time contact capture | Name plus phone or email, declinable; asked once after the first high-intent buyer turn (a stated budget, a time expression with the buyer in the sentence, or a viewing/callback request) or, failing that, by intercepting the first goodbye; second goodbye closes; phone digits are read back deterministically |
...
| P2-S07 | Deterministic chunks, document-level publication and optional per-figure review | Paragraph-aware chunks default admin-only; the reviewer publishes one complete passage manifest while adjacent retrieval chunks are grouped in the UI; bound `project_knowledge` descriptive prose may retrieve only for an inventory project; conflicts/unknown projects stay closed; structured inventory facts remain `inventory_governed`; extracted occurrence list shows value, unit/currency, source sentence and page; only individually approved figures in eligible chunks are active |
