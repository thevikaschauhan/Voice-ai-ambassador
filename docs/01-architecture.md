...
3. **Qwen renders numbers as words** ("two million"), whisper as digits ("2 million"). Any deterministic parse of buyer speech - notably ADR-011's budget confirmation policy, since implemented in `ambassador/budget.py` on exactly this dependency - finds nothing in the qwen transcript.
...
**Resulting budget**, with caching live - and note the first two terms DOUBLE-COUNT, which was not known when this was written: `end_of_utterance_delay` and `transcription_delay` share one VAD anchor, so the endpoint figure already contains the STT figure (measured 2026-09-01: p50 577ms total, of which 466ms is STT; see the nested rows in `docs/04-`). Read the sum below as an upper bound rather than an estimate: endpoint 200-500ms + STT ~280ms + LLM first sentence 530-770ms + guardrail 0.3ms + TTS 327ms after that sentence, so roughly **1.4-1.6s voice-to-voice** against a 1200ms target and 1500ms ceiling. At the ceiling, not comfortably inside it. The preroll masking already specified in `docs/04-` covers the slow tail; further headroom has to come from endpointing and TTS, which are now the two largest remaining components.
...
**Rate limits, and the one model slug that has nowhere to fail over to.** `qwen/qwen3.7-flash` has exactly ONE endpoint on OpenRouter (Alibaba), so provider routing has nothing to route to: a 429 from that shared pool can only be waited out. Measured on the 2026-09-08 call, eleven 429s across six of twelve turns, TTFT rising by roughly 685ms each and the worst turn spending three of its four retry attempts before it answered. No turn failed, so this is headroom rather than a repair. `LLM_FALLBACK_MODELS` sends OpenRouter a `models` array, which it tries when the primary's providers are rate-limited, and `llm_usage.model` reports whichever model actually answered rather than the one that was asked for - without that a fallback would fire invisibly. The `cache_control` breakpoint above is narrowed to the primary slug at the same time, because every fallback candidate reports `supports_implicit_caching: false` and OpenRouter's documentation does not say what a non-caching provider does with the marker. The real fix is a provider key of our own, which leaves the shared pool entirely; that needs a vendor account and is not a code change.
...
Contact capture is deterministic and optional to the buyer. Fixed copy is
asked once after the first high-intent buyer turn - a stated budget, a time
expression with the buyer in the sentence, or a viewing or callback request -
and if no such turn came, the first farewell is intercepted for the one ask and
a second farewell closes immediately. The trigger is DETERMINISTIC and not a
model tool: the budget half is the ADR-011 budget detector's own reading of the
turn, the other two are reviewed word lists, and neither costs a model call.
Both paths consult one `owes_request()`, so whichever fires first spends the
only ask. A buyer who hangs up never says goodbye, which is why the
after-interest trigger exists at all. Name plus phone or email
