...
**Terminal LLM failure** (retry budget exhausted) speaks the composed fallback and escalates regardless of whether audio has already played - unlike a guardrail block, there is nothing left to bridge to when the model is gone. The `llm_failure` event carries `spoken_before` and the `fallback` event carries its reason, so the audit distinguishes the two situations. The retry budget itself is deliberately one bounded layer, ~1-2 seconds worst case including a clamped Retry-After - a frozen turn is a worse outcome than an honest fallback.
