...
- **Automatic mid-call language switching is opt-in.** Set `AUTO_LANGUAGE_SWITCH=true`, `STT_PROVIDER=soniox`, `SONIOX_API_KEY`, and keep `SONIOX_MODEL=stt-rt-v5`. Soniox supplies final language identification; the agent chooses a language only when one language has clear final-transcript evidence, updates the prompt, guardrails, deterministic policies, retrieval query language, brief extractor, and Fish voice in one turn boundary, and emits a closed `response_language_changed` event. Partials never switch the call. The browser still offers only the certified opening languages; the runtime accepts Russian, French, Spanish, Portuguese, Chinese, Japanese, and German as additional switch targets. Those targets remain blocked unless their disclosure is native-authored, or `ALLOW_UNCERTIFIED_LANGUAGE=true` is explicitly enabled for a marked English fallback. Their native guardrail, fallback, disclosure, and policy slots carry `VERIFY:` markers and require review before client-facing use.
