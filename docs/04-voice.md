...
- **The disclosure names the ambassador.** The English copy is "You are speaking with Jane, Binghatti's AI ambassador...", and the name comes from `data/ambassadors.yaml` rather than being written into the sentence. The copy is a TEMPLATE carrying `{name}`, so the Arabic and Hindi sentences a native reviewer writes carry the same placeholder and get the same substitution, and a language whose name is still empty falls back to the unnamed wording that shipped before - which is what `ar` and `hi` do today. A name is product identity chosen by the client, not language copy, so AGENTS.md's rule about not authoring in a language you do not speak does not block setting one; how a given name is written and pronounced in Arabic and Hindi is a different question and it is a reviewer's, which is why the packet asks it. The three commitments the disclosure makes - that this is an AI, that the conversation is transcribed, that a person can be asked for - are unchanged by the name.
