...
- **Automatic mid-call language switching is opt-in, and today it can only switch INTO English.** Set `AUTO_LANGUAGE_SWITCH=true`, `STT_PROVIDER=soniox`, `SONIOX_API_KEY`, and keep `SONIOX_MODEL=stt-rt-v5`. Soniox supplies final language identification; the agent chooses a language only when one language has clear final-transcript evidence, and updates the prompt, guardrails, deterministic policies, retrieval query language, brief extractor, and Fish voice in one turn boundary - including the turn that triggers the switch, because the new prompt is written to the context that turn is generated from as well as to the agent's own. Partials never switch the call. **Two independent barriers refuse a target, and until both are lifted English is the only reachable target: a language with no native-authored disclosure is refused unless `ALLOW_UNCERTIFIED_LANGUAGE=true`, and a language with no authored closing phrases in `data/farewells.yaml` is refused outright** - goodbye detection is keyed on the response language, so switching to a language with none would leave a call the buyer cannot end, and `demo_max_call_seconds=0` means nothing else would end it either. Only English has closing phrases today, so the seven additional codes are unreachable as targets no matter how the certification flag is set. A refusal is on the durable stream as `response_language_switch_skipped` with the reason, once per language rather than once per turn; a switch is `response_language_changed`; a switch that raises is `response_language_switch_failed` and the call continues in the language it was already speaking, so a failure can never cost the buyer their turn. The browser still offers only the certified opening languages; the runtime accepts Russian, French, Spanish, Portuguese, Chinese, Japanese, and German as additional switch targets once their copy exists. Their native guardrail, fallback, disclosure, farewell, and policy slots carry `VERIFY:` markers and require review before client-facing use.
...
Four reasons. `buyer_farewell` and `duration_cap` speak the authored copy (the cap outside any turn, since it can fire when nobody said goodbye). `buyer_left` is a buyer who disconnected and has nobody left to address. `agent_farewell` is the hybrid: the buyer's turn carried a closing PHRASE but not cleanly enough for the strict rule, the model was given the turn and answered with a closing of its own, and two independent readings agreeing is enough to end the call on the goodbye the buyer already heard. Nothing further is spoken there - the model's farewell was the farewell - and the model can never end a call on its own, because without a closing phrase in the buyer's turn there is no candidate to confirm.

`agent_farewell` covers a second shape, added after the 08:32Z call ended on it twice: the model says goodbye and NOTHING ends the call, because the buyer's turn carried no closing phrase for the hybrid to pair it with. That call went "model sign-off" -> "Okay. Thank you." -> another model goodbye -> "Goodbye.", and the buyer had to say goodbye to a service that had already said it. So a sign-off with no close behind it is REMEMBERED for exactly one turn, and a reply on that turn which is nothing but courtesy ends the call. Three restraints make that safe rather than clever. The model's goodbye still cannot end a call on its own - a model that signs off spontaneously would otherwise hang up on a live buyer, which is the asymmetric failure this whole design is built around, so it remains two readings and only the second one has changed shape. A question after a sign-off is a question, and the call goes on. And the memory lasts one turn, because two turns later the conversation has moved on and "thank you" is an ordinary thank you again. The turn is not taken back from the model either: the close is armed and fires on the seal, so the buyer hears the reply they are already owed, and the authored farewell is not spoken for the same reason the hybrid does not speak it.

Phrase tables match on the STEM for a single English plural, in either direction. The same call said "I don't have any further QUESTION" and was refused over the "s" while the plural phrase sat in the table. One inflection only: a looser rule on a table that can end a call is not worth the coverage. Disfluencies ("uh", "um", "er") are courtesies for the same reason "ok" is - a person thinking is not a change of subject, and the recogniser keeps them.

A near miss is recorded either way. `farewell_candidate` carries how many tokens were in the way and whether one of them was the ambassador's name, and never the words themselves: the buyer's closing line is their own text. The first hosted call missed because the client said the ambassador's name, and nothing on the stream could say so.

**Ending the call means deleting the ROOM, not just the job.** `ctx.shutdown()` ends the job; the agent leaves, and a room with a browser still in it lives on, because `departureTimeout` only starts once the last participant has gone. The talk page ends on `ROOM_DELETED`, so it saw nothing and the client pressed End call after a perfectly good goodbye. The room is deleted first and the job shut down after, because deletion is the signal the other side is waiting for - and a failed deletion still shuts the job down, or a transient API error would leave a call up that nobody can end. Local runs cannot catch this: `buyer_publisher` disconnects itself when its script ends, so the room empties either way. Measuring it needs a participant that stays.
...
- **The disclosure names the ambassador.** The English copy is "You are speaking with Jane, Binghatti's AI ambassador...", and the name comes from `data/ambassadors.yaml` rather than being written into the sentence. The client named all three: Jane in English, Nora in Arabic, Maya in Hindi. Each language carries TWO authored sentences, `unnamed` and a `named` one holding `{name}`, so a native reviewer writes both and the substitution never has to invent a form - and there is consequently no rendering path to "You are speaking with , Binghatti's". A language with no `named` sentence yet falls back to its `unnamed` one, which is what `ar` and `hi` do today: they have names but no disclosure at all, so they open on the degraded English path, which says Jane, because the name is looked up on the language actually spoken rather than the one requested. A name is product identity chosen by the client, not language copy, so AGENTS.md's rule about not authoring in a language you do not speak does not block setting one; how each name is WRITTEN in Arabic or Devanagari and how it is said is a different question, it is a reviewer's, and reviewer packet 4b asks it. The three commitments the disclosure makes - that this is an AI, that the conversation is transcribed, that a person can be asked for - are unchanged by the name, which sits inside the first of them rather than beside them.

- **An empty disclosure blocks its language.** A language with no native-authored copy cannot open a call, because the disclosure *is* the thing it would be opening without, so `LANGUAGE=ar` now refuses to start rather than degrading quietly. `ALLOW_UNCERTIFIED_LANGUAGE=true` opens in English instead and marks the event stream `uncertified_fallback: true` - that is the graceful-degradation demo this document argues for, not a way to ship an unreviewed language. Presence of copy in that file is consequently the readiness signal for whether a language may OPEN a call, which makes the ship-Arabic-or-drop-it decision a state of the repository rather than a note in a meeting. It is not, however, the whole readiness picture, and reading it as such is a live trap: every other native-copy table gates its own behaviour independently, so filling only the disclosure buys a call that opens with a properly certified opening and then runs with ADR-011's deterministic confirmation policy switched off (`confirmations.yaml` ar/hi are empty, and `confirmations.covers()` reports the policy off rather than speaking English into the call), no goodbye detection (`farewells.yaml`), the English bridge and fallback standing in (`fallbacks.yaml`), and the numeric guardrail unable to read a magnitude written in Arabic or Devanagari (`numerals.yaml`). Each of those degrades honestly on its own, which is right, but together they add up to a language that sounds certified and is not. The native-reviewer packet asks for all of it in one sitting for exactly this reason, and `tests/test_reviewer_packet.py` fails if a native-copy file is ever added without an ask.
