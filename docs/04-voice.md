...
Two consequences of that trigger are worth stating. A turn nobody could hear is not an answer, so it consumes none of the budget or name policy's three attempts - and while a confirmation is open it is RE-ASKED rather than handed to the model, which is the property ADR-011 exists to hold. And at most one deterministic question is asked per turn, budget first: a project named in the same breath as a budget is not confirmed until the buyer names it again. That is a deliberate trade, not an oversight - a reply meant for one question and read as an answer to another is the defect class that blocked the budget half twice, and one extra mention costs a sentence where a mis-recorded consent costs the demo.
